-- NewsFlow Setup for Existing Database: newsmystimatrix_rep
-- Copy and paste this into phpMyAdmin SQL tab

-- Use your existing database
USE newsmystimatrix_rep;

-- Create user with remote access permissions (if not already exists)
CREATE USER IF NOT EXISTS 'newsmystimatrix_rep'@'%' IDENTIFIED BY 'your_secure_password_here';
...
-- Verify the setup
SELECT 'NewsFlow tables created in newsmystimatrix_rep successfully!' as status;
SELECT COUNT(*) as newsflow_tables FROM information_schema.tables WHERE table_schema = 'newsmystimatrix_rep' AND table_name IN ('sessions', 'users', 'news_sources', 'articles', 'user_articles', 'user_notes', 'reading_history', 'user_preferences');
